...
const entry = readFileSync(new URL('../src/pages/book-intro/index.astro', import.meta.url), 'utf8');
assert.match(entry, /<IntroBooking canonicalPath="\/book-intro\/"/);
...
assert.match(page, /data-booking-version="short-path-20260907"/);
assert.doesNotMatch(page, /data-step=/);
assert.match(page, /href="\/lesson-fit\/guide\/"/);
assert.match(page, /!allowed\.includes\(option\.value\)/);
assert.match(page, /Book Your Free Intro Lesson/);
assert.match(page, /No ongoing commitment/);
assert.match(page, /Our office will contact you to find a suitable teacher and time/);
assert.match(page, /name="preferred_time_window" value="Flexible \/ not sure"/);
assert.doesNotMatch(page, /location office|duplicate account|Save & Continue|Thanks—we’ve got your information/);
...
assert.match(thankYou, /You’re already with us/);
assert.match(thankYou, /Our office will be in touch/);
assert.doesNotMatch(thankYou, /duplicate account|Request Received/);
...
const firstLessonGuide = readFileSync(new URL('../src/content/parent-resources/choosing-the-right-first-music-lesson.md', import.meta.url), 'utf8');
const pricingGuide = readFileSync(new URL('../src/content/parent-resources/music-lesson-cost-cincinnati.md', import.meta.url), 'utf8');
const requestInfoPage = readFileSync(new URL('../request-info.html', import.meta.url), 'utf8');
const windsPage = readFileSync(new URL('../flute-clarinet-lessons.html', import.meta.url), 'utf8');
assert.match(firstLessonGuide, /ctaHref: \/book-intro\//);
assert.match(pricingGuide, /ctaHref: \/book-intro\//);
assert.match(pricingGuide, /\[Schedule an intro lesson\]\(\/book-intro\/\)/);
assert.match(requestInfoPage, /href="\/book-intro\/"[^>]*>Get Started<\/a>/);
...
assert.match(pianoPage, /<IntroBooking canonicalPath="\/book-piano-intro\/" defaultService="piano"/);
...
assert.match(choiceFunction, /deliverRequestInfo/);
